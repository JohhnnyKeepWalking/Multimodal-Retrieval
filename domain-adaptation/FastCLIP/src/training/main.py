...
def main(args):
    ...
    if args.lock_text:
        if args.pretrained == "" or args.pretrained is None:
            model.lock_text_tower(
                unlocked_layers=args.lock_text_unlocked_layers,
                freeze_layer_norm=args.lock_text_freeze_layer_norm)
        else:
            logging.info('Lock text tower from pretrained.')
            try:
                ...
                # fallback: freeze params with "text" in the name
                for name, p in model.named_parameters():
                    if "text" in name or "token_embedding" in name:
                        p.requires_grad = False
